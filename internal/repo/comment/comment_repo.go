...
func (cr *commentRepo) UpdateCommentAI(ctx context.Context, commentID string, isAI bool, aiReplied bool) (err error) {
	_, err = cr.data.DB.Context(ctx).Where("id = ?", commentID).Cols("is_ai", "ai_replied").Update(&entity.Comment{IsAI: isAI, AIReplied: aiReplied})
	return err
}
...
func (cr *commentRepo) UpdateCommentContent(
	...
	_, err = cr.data.DB.Context(ctx).ID(commentID).Cols("original_text", "parsed_text").Update(&entity.Comment{
