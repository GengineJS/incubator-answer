...
type AIReplyType int

const (
	AICommentType AIReplyType = iota
	AIAnswerType
)

func (os *ObjService) UpdateQuesOrAnswerAIReply(ctx context.Context, objectID string, aiReplyType AIReplyType) {
	objInfo, _ := os.GetInfo(ctx, objectID)
	var cols []string
	if objInfo.ObjectType == constant.QuestionObjectType {
		questionInfo, _, _ := os.questionRepo.GetQuestion(ctx, objectID)
		if aiReplyType == AICommentType && !questionInfo.AICommentReplied {
			questionInfo.AICommentReplied = true
			cols = append(cols, "ai_comment_replied")
			os.questionRepo.UpdateQuestion(ctx, questionInfo, cols)
		} else if aiReplyType == AIAnswerType && !questionInfo.AIAnswerReplied {
			questionInfo.AIAnswerReplied = true
			cols = append(cols, "ai_answer_replied")
			os.questionRepo.UpdateQuestion(ctx, questionInfo, cols)
		}
	} else if objInfo.ObjectType == constant.AnswerObjectType {
		answerInfo, _, _ := os.answerRepo.GetAnswer(ctx, objectID)
		if !answerInfo.AIReplied {
			answerInfo.AIReplied = true
			cols = append(cols, "ai_replied")
			os.answerRepo.UpdateAnswer(ctx, answerInfo, cols)
		}
	}
}
...
func (os *ObjService) GetInfo(ctx context.Context, objectID string) (objInfo *schema.SimpleObjectInfo, err error) {
	...
	case constant.CommentObjectType:
		...
		reID := ""
		if commentInfo.ReplyCommentID.Valid {
			reID = strconv.FormatInt(commentInfo.ReplyCommentID.Int64, 10)
		}
		objInfo = &schema.SimpleObjectInfo{
			ObjectID:            commentInfo.ID,
			ObjectCreatorUserID: commentInfo.UserID,
			ObjectType:          objectType,
			IsAI:                commentInfo.IsAI,
