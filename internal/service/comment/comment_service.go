...
type CommentRepo interface {
	...
	UpdateCommentAI(ctx context.Context, commentID string, isAI bool, aiReplied bool) (err error)
...
func (cs *CommentService) UpdateAIReplied(ctx context.Context, objectID string) {
	...
	if objInfo.ObjectType == constant.CommentObjectType {
		...
		if !replyComment.AIReplied {
			replyComment.AIReplied = true
...
func (cs *CommentService) GetCommentFromObject(ctx context.Context, objectID string) (
	resp *schema.SimpleObjectInfo, err error) {
	objInfo, err := cs.objectInfoService.GetInfo(ctx, objectID)
	if err != nil {
		return nil, err
	}
