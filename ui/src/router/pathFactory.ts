...
const questionLanding = (
  ...
  if (!questionId) {
    return slugTitle
      ? `/questions/null/${slugTitle}?content_type=${contentType}`
      : `/questions/null?content_type=${contentType}`;
  }
  // @ts-ignore
  if (/[13]/.test(seo.permalink) && slugTitle) {
    return `/questions/${questionId}/${encodeURIComponent(slugTitle)}?content_type=${contentType}`;
  }
