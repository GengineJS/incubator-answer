...
export function setResumeCtx(task: Task) {
...
export function removeResumeCtx(task: Task | string) {
  const ctxKey = getResumeCtxKey(task);
  try {
...
export function cleanupResumeCtx() {
  // eslint-disable-next-line no-plusplus
...
export function getResumeCtx(task: Task | string): Task | null {
  const ctxKey = getResumeCtxKey(task);
...
export function listResumeCtx(): Task[] {
  const res: Task[] = [];
  // eslint-disable-next-line no-plusplus
  for (let i = 0, len = localStorage.length; i < len; i++) {
    const key = localStorage.key(i);
    if (key && key.startsWith(resumeKeyPrefix)) {
      const value = getResumeCtx(key);
      if (value) {
        res.push(value);
      }
    }
