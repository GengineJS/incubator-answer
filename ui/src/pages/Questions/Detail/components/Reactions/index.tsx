...
interface Props {
  ...
  isAIType?: boolean;
  aiCommented?: boolean;
  handleClickComment: (isAI: boolean) => void;
}
    ...
    <div
      ...
      {showAddCommentBtn && (
        <Button
          ...
          size="sm"
          onClick={() => {
            handleClickComment(false);
          }}>
          <Icon name="chat-text-fill" />
          <span className="ms-1">
            {mode !== 'question'
              ? t('comment.btn_add_comment')
              : t('comment.btn_add_subject_comment')}
          </span>
        </Button>
      )}
      {isAIType &&
        (!aiCommented ? (
          <Button
            ...
            <Icon name="chat-text-fill" />
            <span className="ms-1">{t('comment.btn_ai_add_comment')}</span>
          </Button>
        ) : (
          <div className="lh-1 m-2">
            <Badge bg="secondary" pill>
              <Icon name="check-circle-fill  me-1" />
              {t('comment.btn_dis_ai_reply')}
            </Badge>
          </div>
        ))}
